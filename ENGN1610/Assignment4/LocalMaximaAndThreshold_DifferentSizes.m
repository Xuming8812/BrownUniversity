function [J,R] = LocalMaximaAndThreshold_DifferentSizes(I,threshold,length,width)
    ...
    %               R----Type: matrix of double; Meaning: the matrix
    %                       storing the local maximas results, including
    %                       matching value, coordinate, window size for
    %                       further operation
    ...
    %store the information of local maximas
    R = zeros(count,5);
    count = 0;
    ...
    for i = 1 : rowNum_I
        for j = 1 : colNum_I
            if J(i,j) == 1
               count = count +1;
               R(count,1) = I(i,j);             %matching value
               R(count,2) = i;                  %row of local maxima
               R(count,3) = j;                  %column of local maxima
               R(count,4) = length;             %length of window
               R(count,5) = width;              %width of window
            end
        end
    end
