...
# def test_svm_different_parameter(train_data, test_data, lambda_param=.1):
#     """
#     Create an SVM classifier with a specificied kernel_func, train it with
#     train_data and print the accuracy of model on test_data
...
#         svm_model = SVM(kernel_func=rbf_with_gamma, lambda_param=lambda_param)
#         svm_model.train(train_data.inputs, train_data.labels)
#         train_accuracy = svm_model.accuracy(train_data.inputs, train_data.labels)
#         test_accuracy = svm_model.accuracy(test_data.inputs, test_data.labels)
...
#     plt.plot(gammas, train_accuracy_list,label='train accuracy')
#     plt.plot(gammas, test_accuracy_list, label='test accuracy')
#     plt.xscale('log')
#     plt.legend()
#     plt.show()

# def test_svm_different_parameter_polynomial(train_data, test_data, lambda_param=.1):
#     """
#     Create an SVM classifier with a specificied kernel_func, train it with
#     train_data and print the accuracy of model on test_data
...
#     :return: None
#     """
#     paras = np.linspace(0,5,20)

#     train_accuracy_list = []
#     test_accuracy_list = []
#     for c in paras:
#         poly_with_d = lambda x, y: polynomial_kernel(x, y, c = c, d=2)
#         svm_model = SVM(kernel_func=poly_with_d, lambda_param=lambda_param)
#         svm_model.train(train_data.inputs, train_data.labels)
#         train_accuracy = svm_model.accuracy(train_data.inputs, train_data.labels)
#         test_accuracy = svm_model.accuracy(test_data.inputs, test_data.labels)
#         train_accuracy_list.append(train_accuracy)
...
#     plt.plot(paras, train_accuracy_list,label='train accuracy')
#     plt.plot(paras, test_accuracy_list, label='test accuracy')
#     plt.legend()
#     plt.show()
...
def main():
    ...
    #test_svm_different_parameter(train_data, test_data,lambda_param)
    #test_svm_different_parameter_polynomial(train_data, test_data,lambda_param)
